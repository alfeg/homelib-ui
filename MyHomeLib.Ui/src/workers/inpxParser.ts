...
function decodeText(bytes: Uint8Array): string {
    ...
    if (cp1251Decoder) {
        try { return cp1251Decoder.decode(bytes); } catch { /* ignored */ }
    }
...
export function parseInpxBuffer(buffer: ArrayBuffer, onProgress?: ParseProgressCallback): ParseResult {
    ...
    for (let ei = 0; ei < inpEntries.length; ei++) {
        ...
        for (const line of lines) {
            ...
            books.push(book);
            sigHash = updateSigHash(sigHash, `${book.id}|${book.title}|${book.authors}|${book.series}|${book.lang}|${book.genre}|${book.file}|${book.archiveFile}|${book.ext}`);
        }
