...
function progressText() {
  ...
  if (props.progress.phase === "indexing") {
    return t("status.indexing", { processed: props.progress.processed, total: props.progress.total, percent: props.progress.percent });
  }

  if (props.progress.phase === "parsing") {
    if (props.progress.total) {
      return t("status.parsingWithTotal", { processed: props.progress.processed, total: props.progress.total, percent: props.progress.percent });
    }
...
<template>
  <header class="flex justify-between gap-4 items-start mb-4 flex-col lg:flex-row">
    <div>
      ...
      <p class="text-base-content/70">{{ t("common.hash") }}: <span class="font-mono">{{ hash }}</span></p>
      <p class="text-base-content/70" v-if="metadata">{{ t("common.versionBooks", { version: metadata.version, count: metadata.totalBooks }) }}</p>
      <p class="text-base-content/70" v-if="lastUpdatedAt">{{ t("common.lastUpdate", { value: lastUpdatedAt }) }}</p>
    </div>
 ...
 <svg
    ...
    <path
      ...
      <span class="badge badge-sm uppercase tracking-wide" :class="phaseStatus().cls">
        {{ phaseStatus().text }}
      </span>
      ...
      <button class="btn btn-outline btn-error btn-sm" @click="$emit('reset')">{{ t("buttons.fullReset") }}</button>
    </div>
  </header>
