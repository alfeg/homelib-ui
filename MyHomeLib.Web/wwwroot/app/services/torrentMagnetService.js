...
function parseByteString(bytes, offset) {
    const start = offset;
...
function parseInteger(bytes, offset) {
    if (bytes[offset] !== 105) {
...
function parseValue(bytes, offset) {
...
function extractTrackers(announceValue, announceListValue) {
    const trackers = [];
...
function parseTorrentMetadata(bytes) {
    ...
    const displayName = decodeUtf8(infoDict["name.utf-8"] instanceof Uint8Array ? infoDict["name.utf-8"] : infoDict.name);
...
function buildMagnetUri(hash, metadata) {
...
export async function convertTorrentFileToMagnet(file) {
