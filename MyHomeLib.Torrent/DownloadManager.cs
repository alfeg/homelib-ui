    ...
    ILogger<DownloadManager> logger)
{
    ...
    {
        var connected = await torrServe.CheckConnectionAsync(ct);
        if (!connected)
        {
            _stats = new TorrentStats(false, "TorrServe unreachable");
            return;
        }
        var t = await torrServe.GetTorrentAsync(hash, ct);
        var cache = string.IsNullOrEmpty(hash) ? null : await torrServe.GetCacheAsync(hash, ct);
        _stats = new TorrentStats(
            ...
            BitRate:           t?.BitRate           ?? "",
            TorrServVersion:   torrServe.LastVersion ?? ""
        );
    }
